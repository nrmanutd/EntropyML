...
from CodeResearch.LearningFramework.Learners.NNUpdatableTorchModelLearner import NNUpdatableTorchModelLearner
...
class Cifar100ForgettingLearnerFactory(PyTorchCVCLLearnersFactory):
    def __init__(self, nClasses, targetBatchSize=128, scoringBatchSize=64):
        super().__init__(nClasses, targetBatchSize, scoringBatchSize)
...
class Cifar100CumulativeLearnerFactory(PyTorchCVCLLearnersFactory):
    def __init__(self, nClasses, targetBatchSize=128, scoringBatchSize=64):
        super().__init__(nClasses, targetBatchSize, scoringBatchSize)
    ...
    def createTargetLearner_int(self, epochs):
        cifar100_target_learner = NNCumulativeEpochLearner(
            model_factory=lambda: ResNet18CIFAR(num_classes=self.nClasses, width_mult=1.0),
            optimizer_name="sgd",
            lr=0.1,  # классика для CIFAR при bs~128
            weight_decay=5e-4,
...
class Cifar100CachedOptimizerLearnerFactory(PyTorchCVCLLearnersFactory):
    def __init__(self, nClasses, targetBatchSize=128, scoringBatchSize=64):
        super().__init__(nClasses, targetBatchSize, scoringBatchSize)

    def createScoreLearner_int(self, epochs):
        # scoring: сеть меньше, lr ниже, чтобы метрика была стабильнее
        cifar100_scoring_learner = NNUpdatableTorchModelLearner(
            model_factory=lambda: CifarResNet6n2(num_classes=self.nClasses, n=3, width_mult=0.5),
