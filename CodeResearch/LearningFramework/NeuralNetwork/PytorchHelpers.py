...
class ResNet34CIFAR(nn.Module):
    """
    ResNet34 с CIFAR-подобным stem:

        Conv3x3, stride=1
        без начального MaxPool

    Подходит для:
        CIFAR-10:       3 x 32 x 32
        CIFAR-100:      3 x 32 x 32
        Tiny ImageNet:  3 x 64 x 64

    Структура residual-групп:
        [3, 4, 6, 3]
    ...
    def __init__(
        ...
        if width_mult <= 0:
            raise ValueError(
                f"width_mult должен быть положительным, "
                f"получено {width_mult}"
            )
        ...
        if base < 1:
            raise ValueError(
                f"После применения width_mult число каналов равно {base}"
            )

        self.in_ch = base

        # В отличие от стандартной ImageNet ResNet:
        # kernel_size=3, stride=1 и нет MaxPool.
    ...
    def _initialize_weights(self):
        """
        Инициализация, стандартная для ResNet.
        """
